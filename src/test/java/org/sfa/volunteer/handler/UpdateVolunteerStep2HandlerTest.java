...
class UpdateVolunteerStep2HandlerTest {
    ...
    private UpdateVolunteerStep2Handler handler;
    ...
    void setUp() throws Exception {
        ...
        handler = new UpdateVolunteerStep2Handler();
    ...
    private void resetMockInjection() {
        try {
            Field volunteerServiceField = UpdateVolunteerStep2Handler.class.getDeclaredField("volunteerService");
        ...
        try {
            Field responseBuilderField = UpdateVolunteerStep2Handler.class.getDeclaredField("responseBuilder");
    ...
    void handleRequest_ShouldReturnCreatedResponse_WhenRequestIsValid() throws Exception {
        ...
        String requestBody = "{\"step\":2,\"userId\":\"user123\",\"govtIdFilename\":\"passport.pdf\"}";
        ...
        VolunteerResponse volunteerResponse = VolunteerResponse.builder()
                .userId("user123")
                .govtIdFilename("passport.pdf")
                .govtUpdateDate(ZonedDateTime.now())
                .build();
        ...
        when(volunteerService.updateVolunteerStep2(any(VolunteerRequest.class)))
                .thenReturn(volunteerResponse);
        
        System.out.println("[TRACE] Mock setup complete for updateVolunteerStep2");
        ...
        assertEquals("passport.pdf", parsed.data().govtIdFilename());

        verify(volunteerService, times(1)).updateVolunteerStep2(any(VolunteerRequest.class));
    ...
    void handleRequest_ShouldReturnBadRequest_WhenEnumUnspecifiedExceptionOccurs() throws Exception {
        ...
        when(volunteerService.updateVolunteerStep2(any(VolunteerRequest.class))).thenThrow(exception);
    ...
    void handleRequest_ShouldReturnBadRequest_WhenInvalidRequestExceptionOccurs() throws Exception {
        // Arrange
        String requestBody = "{\"step\":2,\"userId\":\"user123\",\"govtIdFilename\":\"passport.pdf\",\"govtUpdateDate\":\"2025-12-09T00:00:00Z\"}";
    ...
    void handleRequest_ShouldHandleInvalidJsonBody() throws Exception {
        ...
        verify(volunteerService, never()).updateVolunteerStep2(any(VolunteerRequest.class));
        verify(responseBuilder, never()).buildSuccessResponse(any(), any(), any());
    }
    ...
    void handleRequest_ShouldHandleMissingRequiredFields() throws Exception {
        // Arrange
        String requestBody = "{\"step\":2}"; // Missing userId
        APIGatewayProxyRequestEvent requestEvent = new APIGatewayProxyRequestEvent()
                .withHeaders(Collections.singletonMap("Accept-Language", "en"))
                .withBody(requestBody);
    ...
    void handleRequest_ShouldHandleDifferentLocales() throws Exception {
        ...
        for (Map.Entry<String, String> localeEntry : locales.entrySet()) {
            ...
            when(volunteerService.updateVolunteerStep2(any(VolunteerRequest.class))).thenReturn(volunteerResponse);
            when(responseBuilder.buildSuccessResponse(
                    eq(SaayamStatusCode.VOLUNTEER_UPDATED),
                    any(Object[].class),
                    eq(volunteerResponse))
            ...
            verify(volunteerService, atLeastOnce()).updateVolunteerStep2(any(VolunteerRequest.class));
        }
    }
    ...
    void handleRequest_ShouldParseRequestCorrectly() throws Exception {
        // Arrange
        String requestBody = "{\"step\":2,\"userId\":\"user123\",\"govtIdFilename\":\"passport.pdf\",\"notification\":false}";
        APIGatewayProxyRequestEvent requestEvent = new APIGatewayProxyRequestEvent()
                .withHeaders(Collections.singletonMap("Accept-Language", "en"))
                .withBody(requestBody);
        ...
        when(volunteerService.updateVolunteerStep2(any(VolunteerRequest.class))).thenAnswer(invocation -> {
            VolunteerRequest request = invocation.getArgument(0);
            assertEquals(2, request.step());
            assertEquals("user123", request.userId());
            assertEquals("passport.pdf", request.govtIdFilename());
    ...
    void handleRequest_ShouldHandleMalformedJsonGracefully() throws Exception {
        ...
        String[] malformedJsonBodies = {
                "{step:2}",  // Missing quotes
                "{\"step\":}",  // Missing value
                "{'step':2}",  // Single quotes
                "step=2",  // Not JSON
                "{\"step\":2,\"userId\":}",  // Incomplete
        };
