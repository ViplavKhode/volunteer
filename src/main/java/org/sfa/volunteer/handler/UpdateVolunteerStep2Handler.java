...
@Slf4j
public class UpdateVolunteerStep2Handler extends BaseRequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent requestEvent, Context lambdaContext) {
        ...
        try {
            log.info("Received update step 2 request event: {}", requestEvent);


            Map<String, Object> body = parseBody(requestEvent.getBody());
            VolunteerRequest updateRequest = parseRequest(body);

            VolunteerResponse updatedVolunteer = volunteerService.updateVolunteerStep2(updateRequest);
            ...
            log.info("Update step 2 request successful. Response: {}", successResponse);
