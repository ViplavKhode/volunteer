...
@Slf4j
public class UpdateVolunteerStep3Handler extends BaseRequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent requestEvent, Context lambdaContext) {
        ...
        try {
            log.info("Received update step 3 request event: {}", requestEvent);


            Map<String, Object> body = parseBody(requestEvent.getBody());
            VolunteerRequest updateRequest = parseRequest(body);

            VolunteerResponse updatedVolunteer = volunteerService.updateVolunteerStep3(updateRequest);
            ...
            log.info("Update step 3 request successful. Response: {}", successResponse);
